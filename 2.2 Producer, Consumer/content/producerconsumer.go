...
// Producer is a type for structs that holds two channels: one for pizza, with all
...
func makePizza(pizzaNumber int) *PizzaOrder {
	...
	if pizzaNumber <= NumberOfPizzas {
		...
		if rnd <= 2 {
			...
			msg = fmt.Sprintf("*** We cook quit while making pizza #%d!", pizzaNumber)
		} else {
			success = true
...
// for the loop. It executes until it receives something on the quit
