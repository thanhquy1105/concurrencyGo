...
var philosophers = []Philosopher{
	{name: "AAA", leftFork: 4, rightFork: 0},
	{name: "BBB", leftFork: 0, rightFork: 1},
	{name: "CCC", leftFork: 1, rightFork: 2},
	{name: "DDD", leftFork: 2, rightFork: 3},
	{name: "EEE", leftFork: 3, rightFork: 4},
}

// define some variables
var hunger = 3                  // how many times does a person eats
...
func dine() {
	...
	// when it reaches zero. everone is finished eating and has left. We add 5 (the number of
	// philosophers) to this wait group
	wg := &sync.WaitGroup{}
	...
	// in the Philosopher type. Each fork, then, can be found using the index (an integer),
	// and each fork has a unique mutex.
	var forks = make(map[int]*sync.Mutex)
	for i := 0; i < len(philosophers); i++ {
		forks[i] = &sync.Mutex{}
	}

	// start the meal by iterating through out slice of Philosophers.
	for i := 0; i < len(philosophers); i++ {
		// fire off a goroutine for the current philosopher
		go diningProblem(philosophers[i], wg, forks, seated)
	}

	// Wait for the philosophers to finish. This blocks until the wail group is 0
	wg.Wait()
}
...
func diningProblem(philosopher Philosopher, wg *sync.WaitGroup, forks map[int]*sync.Mutex, seated *sync.WaitGroup) {
	...
	// eat three times. have this philosopher eatTime and thinkTime "hunger" times (3)
	for i := hunger; i > 0; i-- {
		...
		// Node that the goroutine will block (pause) until it gets a lock on both the right and left fork.
		if philosopher.leftFork > philosopher.rightFork {
			forks[philosopher.rightFork].Lock()
			fmt.Printf("\t%s takes the right %d fork.\n", philosopher.name, philosopher.rightFork)
			forks[philosopher.leftFork].Lock()
			fmt.Printf("\t%s takes the left %d fork.\n", philosopher.name, philosopher.leftFork)
		} else {
			forks[philosopher.leftFork].Lock()
			fmt.Printf("\t%s takes the left %d fork.\n", philosopher.name, philosopher.leftFork)
			forks[philosopher.rightFork].Lock()
